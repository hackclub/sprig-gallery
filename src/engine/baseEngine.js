...
export function baseEngine() {
  ...
  const _canMoveToPush = (sprite, dx, dy) => {
    ...
    grid[i].forEach((sprite) => {
      ...
      if (isSolid && !isPushable) canMove = false;
  ...
  const _checkLegend = (type) => {
    if (!(type in Object.fromEntries(state.legend))) throw new Error(`Unknown sprite type: ${type}`);
  };
  ...
  const hasTypeAny = (x, y, types) =>
    getTile(x, y)
      .map((sprite) => sprite.type)
      .some((type) => types.includes(type));

  const hasTypeAll = (x, y, types) =>
    getTile(x, y)
      .map((sprite) => sprite.type)
